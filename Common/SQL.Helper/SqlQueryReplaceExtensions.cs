...
{
    public static class SqlQueryReplaceExtensions
    {
        ...
        public static string ReplaceQueryParameters(this string query, string schemaName, string tableName, string parameterNotation, string[] parameterPlaceholders, string[] sqlParameterValues, string whereCluse = "")
        {
            var queryBuilder = new StringBuilder(query);
            for (int i = 0; i < parameterPlaceholders.Length; i++)
                queryBuilder.Replace($"{parameterPlaceholders[i]}", $"{parameterNotation}{sqlParameterValues[i]}");
            ...
            if (whereCluse != string.Empty)
            {
                queryBuilder.Append(whereCluse);
            }
